...
################################################################################
# Based on https://github.com/simontomaskarlsson/CycleGAN-Keras/blob/master/model.py
################################################################################
...
#         sys.stdout.flush()
        ...
        if self.use_data_generator:
            self.data_generator=crop_generator
#         if self.use_data_generator:
...
#===============================================================================
# Training
    def train(self, epochs, steps_per_epoch, batch_size=1, save_interval=1):
...
#                 print(real_images_A.shape,ones.shape)
            ...
            for _ in range(self.generator_iterations):
                print(real_images_A.shape,real_images_B.shape)
...
#             print(ones)
        ...
        sess = tf.Session()
        crop_generator = self.crop_generator.generate2domains(sess)
        for epoch in range(1, epochs + 1):
            if self.use_data_generator:
                loop_index = 1
#                 for images in self.data_generator:
#                 print(steps_per_epoch)
#                 steps_per_epoch=crop_generator.__len__()
                steps_per_epoch2=batch_size
                for step in range(steps_per_epoch2):
                    images = next(crop_generator)
...
#                     print(ones)
                    run_training_iteration(loop_index, steps_per_epoch2)
                    ...
                    if loop_index >= steps_per_epoch2:
                        break
...
#         print(self.config["paths"]["root"])
        directory = os.path.join(self.config["paths"]["root"]+'/outputs/images', self.date_time)
        
#         print('savedImagesDir',directory)
...
#         for i in range(num_saved_images + 1):
        for i in range(num_saved_images):
            ...
            self.truncateAndSave(real_image_Ab, real_image_A, synthetic_image_B, reconstructed_image_A,directory+
                                 '/{}/epoch{}_sample{}.png'.format(
                                     'A' + testString, epoch, i))
            self.truncateAndSave(real_image_Ba, real_image_B, synthetic_image_A, reconstructed_image_B,directory+
                                 '/{}/epoch{}_sample{}.png'.format(
                                     'B' + testString, epoch, i))
...
# Save and load
    ...
    def saveModel(self, model, epoch):
        ...
        directory = os.path.join(self.config["paths"]["checkpoints"],'saved_models', self.date_time)
#         print('savedModelDir',directory)
        if not os.path.exists(directory):
            os.makedirs(directory)

        model_path_w = directory+'/{}_weights_epoch_{}.hdf5'.format(model.name, epoch)
        model.save_weights(model_path_w)
        model_path_m = directory+'/{}_model_epoch_{}.json'.format(model.name, epoch)
    ...
    def writeLossDataToFile(self, history):
        ...
        diir=self.config["paths"]["root"]+'/outputs/images'+'/{}/loss_output.csv'.format(self.date_time)
        with open(diir, 'w') as csv_file:
    ...
    def writeMetaDataToJSON(self):
        ...
        with open(self.config["paths"]["root"]+'/outputs/images'+'/{}/meta_data.json'.format(self.date_time), 'w') as outfile:
...
#  train(self, epochs, batch_size=1, save_interval=1):
# self.train(epochs=self.epochs, batch_size=self.batch_size, save_interval=self.save_interval)
#     GAN = CycleGAN()
class ModelClass(DeepProfilerModel):
    def __init__(self, config, dset, generator, val_generator):
        ...
        self.gan = CycleGAN(config, self.train_crop_generator, self.val_crop_generator)
#         self.feature_model = self.gan.modelDiscriminator
...
#         self.gan.combined.summary()
        ...
        # ======= Create designated run folder and store meta data ==========
#         directory = os.path.join('images', self.gan.date_time)
#         if not os.path.exists(directory):
#             os.makedirs(directory)
#         self.gan.writeMetaDataToJSON()
...
#         self.gan.train(epochs, batch_size=self.gan.batch_size, save_interval=self.gan.save_interval)
        self.gan.train(epochs, steps,batch_size=self.gan.batch_size, save_interval=self.gan.save_interval)
